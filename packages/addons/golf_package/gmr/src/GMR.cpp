...
bool GaussianMixture::loadParams(const char fileName[])
{
  ...
  fich >> dim >> nState;
  inComponents.Resize(dim/2);
  outComponents.Resize(dim/2);
  for (int i=0; i<dim/2; i++)
  {
     inComponents[i] = i;
	 outComponents[i] = i + dim/2;
  }
...
double GaussianMixture::stepEM(Matrix DataSet, bool update_covariances)
{
  ...
  for(int j=0;j<nState;j++) 
  {
    ...
    DataSet.TransposeMult(pix.GetColumn(j),tmu); // new mean for state j
    ...
    if(update_covariances) 
    {
       ...
       tmptmu.TransposeMult(tmptmu.MultRow(pix.GetColumn(j)) ,sigma[j]);
  ...
  for(int i = 0;i<nState;i++)
    {
      ...
      if((!sigma[i].IsInverseOk())||isnan(detSigma[i])){
        // Add some stuff in the diagonal
        Matrix eye(sigma[i].RowSize(),sigma[i].RowSize());
        eye.Identity();
        eye*=1e6;
        sigma[i]+=eye;
        sigma[i].InverseSymmetric(invSigma[i], &detSigma[i]);
...
Matrix GaussianMixture::doRegression(Matrix in,Matrix * SigmaOut)
{
...
Vector GaussianMixture::getRegressionOffset( Vector in, 
    ...
    regOutM = doOffsetRegression(inM,offset,&sigma,inPriorM,priorComponents);
...
Vector GaussianMixture::doRegressionPartial(Vector in,Matrix& Sigma,Vector states)
{
  int outDim = outComponents.Size();
