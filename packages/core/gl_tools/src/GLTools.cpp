...
int GLTools::sDrawRefCallListId = 0;
...
void GLTools::DrawRef(float scale, const Matrix4 *ref){
  ...
  if(sDrawRefCallListId==0){
    sDrawRefCallListId = glGenLists(1);

    if(sDrawRefCallListId>0)
        glNewList(sDrawRefCallListId,GL_COMPILE);
  }else{
      bNeedRender =false;
  }
