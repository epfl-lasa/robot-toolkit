...
unsigned char *ImageIO::LoadRGB(const char *name, int *width, int *height, int *components){
   unsigned *base, *lptr;
...
bool ImageIO::SaveRGB(const char *filename, unsigned char *buffer, int width, int height, int components){
    FILE* m_File = fopen(filename, "wb");
    if(m_File){
        unsigned char** imData = new unsigned char*[components];
        unsigned char** imDataPtr = new unsigned char*[components];
        for(int i=0;i<components;i++){
            imData[i]    = (unsigned char*)malloc(width*height*sizeof(unsigned char));
            imDataPtr[i] = imData[i];
        }
        ...
        PUT_SHORT(m_File,(short)width);         /* XSIZE                       */
        PUT_SHORT(m_File,(short)height);         /* YSIZE                       */
        PUT_SHORT(m_File,(short)components);         /* ZSIZE                       */
        ...
        for(int i=0;i<components;i++)
            fwrite(imData[i],width*height,1,m_File);
