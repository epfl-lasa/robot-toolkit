    ...
    */
    ...
    inline Vector3& GetNormRotationAxis(Vector3 &result) const{
        ...
        if(norm>EPSILON)
            ...
            result.Zero();
            //added by Mohito to avoid some bugs in the equation above
            if (_[0*3+0]+_[1*3+1]+_[2*3+2] < -0.9999){
                for(int i=0;i<3;i++){
                    if (_[i*3+i]==1)
                        result[i] = 1;
                }
    ...
    /// Get the rotation axis of a rotation matrix (the norm equals 1)
        inline Vector3 GetNormRotationAxis() const{
        	Vector3 result;
        	return GetNormRotationAxis(result);
        }
    ...
    inline void GetQuaternionRepresentation(Vector & result){
      if(result.Size() != 4)
        result.Resize(4);
    
      //  result[0] = 0.5*sqrt(1+R(0,0)+R(1,1)+R(2,2));
      result[0] = 0.5*sqrt(1+_[0*3+0]+_[1*3+1]+_[2*3+2]);
      //  result[1] = 0.5*sign(R(2,1)-R(1,2))*(sqrt(abs(1.0+R(0,0)-R(1,1)-R(2,2))));
      result[1] = 0.5*sign(_[2*3+1]-_[1*3+2])*(sqrt(fabs(1.0+_[0*3+0]-_[1*3+1]-_[2*3+2])));
      //  result[2] = 0.5*sign(R(0,2)-R(2,0))*(sqrt(abs(1.0-R(0,0)+R(1,1)-R(2,2))));
      result[2] = 0.5*sign(_[0*3+2]-_[2*3+0])*(sqrt(fabs(1.0-_[0*3+0]+_[1*3+1]-_[2*3+2])));
      //  result[3] = 0.5*sign(R(1,0)-R(0,1))*(sqrt(abs(1.0-R(0,0)-R(1,1)+R(2,2))));
      result[3] = 0.5*sign(_[1*3+0]-_[0*3+1])*(sqrt(fabs(1.0-_[0*3+0]-_[1*3+1]+_[2*3+2])));

    }

    /* inline Matrix3& GetQuaternionRepresentation(REALTYPE& eta,Vector3& eps){ */
    /*   REALTYPE S; */
    /*   REALTYPE m00 = this->At(0,0); */
    /*   REALTYPE m01 = this->At(0,1); */
    /*   REALTYPE m02 = this->At(0,2); */
    /*   REALTYPE m10 = this->At(1,0); */
    /*   REALTYPE m11 = this->At(1,1); */
    /*   REALTYPE m12 = this->At(1,2); */
    /*   REALTYPE m20 = this->At(2,0); */
    /*   REALTYPE m21 = this->At(2,1); */
    /*   REALTYPE m22 = this->At(2,2); */
    /*   REALTYPE T = m00+m11+m22; */
        
    /*   if(T>0){//perform direct computation */
    /*     S = sqrt(T+1)*2; */
    /*     eta = 0.25*S; */
    /*     eps(0) = m21 - m12; */
    /*     eps(1) = m02 - m20; */
    ...
    /*   //      find largest to smallest uvw diagonal elements */
    /* /\*   double u=0; *\/ */
    /* /\*   double v=0; *\/ */
    /* /\*   double w=0; *\/ */
    /* /\*   int ui,vi,wi; *\/ */
    ...
    /* /\*     } *\/ */
    /* /\*   } *\/ */
    /* /\*   double r; // value used in computation *\/ */
    /* /\*   r = sqrt(1+u-v-w);//note that this is always well defined due to the permutation *\/ */
    /* /\*   if(r<EPSILON){ *\/ */
    /* /\*     eta = 1; *\/ */
    /* /\*     eps.Zero(); *\/ */
    /* /\*     // this is a robust way of computing the quaternion *\/ */
    /* /\*     eta = (this->At(wi,vi) - this->At(vi,wi)) / (2*r); *\/ */
    /* /\*     eps(ui) = r/2; *\/ */
    /* /\*     eps(vi) =  (this->At(ui,vi) + this->At(vi,ui)) / (2*r); *\/ */
    /* /\*     eps(wi) =  (this->At(wi,ui) + this->At(ui,wi)) / (2*r); *\/ */
