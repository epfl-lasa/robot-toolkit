...
void UDPConnection::Clear(){
  ...
  memset(m_buffer,         0, MAX_FULL_BUFFER_SIZE * sizeof(unsigned char));
  memset(m_pingBuffer,     0, MAX_BUFFER_SIZE      * sizeof(unsigned char));
}
...
bool UDPConnection::IsInitialized(){
    return (m_ConMode!=CONMODE_NONE);
}
...
int UDPConnection::SendMessage(MsgType type, const void *data, int data_len){
  ...
  unsigned int tmp_data_len = data_len + sizeof(long int);

  if(data_len>0){
    tmp_data_len = (tmp_data_len > MAX_FULL_BUFFER_SIZE ? MAX_FULL_BUFFER_SIZE : tmp_data_len);
...
int UDPConnection::Ping(const void *data, int data_len){
  if(!SendMessage(PING_CONNECTION,data,data_len))
    return FALSE;

  return TRUE;
}

int UDPConnection::Pong(const void *data, int data_len){
  if(!SendMessage(PONG_CONNECTION,data,data_len))
    return FALSE;
...
int UDPConnection::GetData(void * data, int max_data_len, bool block, bool flush){
  ...
  if(flush) block = false;
    ...
    if(flush){
        while(GetMessage(&type,data,&data_len,block)){
            if(type == SEND_DATA){
                bGotMsg= true;
            }
        }
...
char* UDPConnection::GetPingPongBuffer(){
    return m_pingBuffer;
}
...
char* UDPConnection::GetClientIP(){
    long int adr = (long int)m_client.sin_addr.s_addr;
    sprintf(mClientIP,"%d.%d.%d.%d",((unsigned char*)&adr)[0],((unsigned char*)&adr)[1],((unsigned char*)&adr)[2],((unsigned char*)&adr)[3]);
    return mClientIP;
}
...
void UDPConnection::ClearAllowedClients(){
    m_nb_allowed_client = 0;
}
